...
async def search(
    query: str, index: InvertedIndex, writer: asyncio.StreamWriter  # <1>
) -> int:
    chars = index.search(query)  # <2>
    lines = (line.encode() + CRLF for line in format_results(chars))  # <3>
...
async def supervisor(index: InvertedIndex, host: str, port: int) -> None:
    server = await asyncio.start_server(  # <1>
        functools.partial(finder, index), host, port  # <2>
    )  # <3>
