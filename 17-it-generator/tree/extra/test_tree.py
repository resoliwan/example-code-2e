...
def test_1_level():
    ...
    expected = [("One", 0, True)]
    result = [(cls.__name__, level, last) for cls, level, last in tree(One)]
...
def test_2_levels_2_leaves():
    ...
    result = [(cls.__name__, level, last) for cls, level, last in tree(Branch)]
    assert expected == result
...
def test_4_levels_1_leaf():
    ...
    result = [(cls.__name__, level, last) for cls, level, last in tree(Level0)]
    assert expected == result
...
def test_4_levels_3_leaves():
    ...
    result = [(cls.__name__, level, last) for cls, level, last in tree(A)]
    assert expected == result
...
def test_many_levels_1_leaf():
    ...
    result = [(cls.__name__, level, last) for cls, level, last in tree(Root)]
